...
namespace Bosh\CloqueBundle\Command;
...
class GitHookPreReceiveCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $compileDir = sprintf(
            '%s/compiled/%s/',
            $this->getContainer()->getParameter('bosh_cloque.versioning.repository.path_prefix'),
            $this->getContainer()->getParameter('bosh_cloque.director_name')
        );

        $p = new Process(
            sprintf(
                '%s --git-dir=%s diff --name-only %s..%s -- %s',
                $this->getContainer()->getParameter('bosh_cloque.executable.git'),
                escapeshellarg($this->getContainer()->getParameter('bosh_cloque.versioning.repository.workspace')),
